...
import {
  importBulkProductImage,
  previewBulkProductImages,
} from "@/lib/products/bulk-image-import";
...
export async function POST(request: NextRequest) {
  ...
  if (contentType.includes("application/json")) {
    ...
    const rows = await previewBulkProductImages(filenames, {
  ...
  const result = await importBulkProductImage(
    {
      filename: file.name,
      buffer,
    ...
    {
      append: formData.get("append") === "true",
